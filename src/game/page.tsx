...
export default function GamePage() {
    ...
    if (answerSelected===answer) {
        ...
        setAllRightQuestions((prev) => {
            return [...prev, {...currentQuestion, answer:answer, studentInput: answerSelected}]
        })
        return true;
    } else {
        setWasRight(false);
  ...
  const [lobby, setLobby] = useState<Player[]>([
    {
        ...
        imgUrl: "https://img.daisyui.com/images/profile/demo/distracted1@192.webp",
        isMuted: false,
    },
    {
        id: "12390123",
  ...
  return (
    ...
    <div className="grid grid-cols-6 items-start relative justify-start w-screen h-full">
        ...
        {(popupAnswer && typeof wasRight == "boolean") && (
            <AnswerPopUp answer={popupAnswer} wasRight={wasRight} scoreChange={wasRight ? "+0.1" : "-0.4"} />

        )}
   
        <div className="col-span-3 w-full h-full">
            
            <Game coconutMultiplier={coconutMultiply} />
        ...
        <div className="col-span-3 w-full h-full relative">
            <div className="absolute w-full">
                <div className="w-5/6 mx-auto">
                    ...
                    <MultipleChoice question={currentQuestion} setAnswerSaved={setAnswerSelected} />
                    ...
                    <div className="mt-8 font-1">
                        ...
                        {lobby.map((player,i) => (
                             <div key={i} className="flex rounded-box items-center gap-3 flex-col mt-2 p-5 bg-base-300 text-center">
                            ...
                            {!player.isMuted ?
                             <button onClick={() => setLobby((prev) => {
                                const newLobby = [...prev];
                                newLobby[i] = { ...newLobby[i], isMuted: !newLobby[i].isMuted };
                                return newLobby;
                             })} className="btn btn-ghost btn-error btn-circle"><BiMicrophone className="size-6" /></button>
                             :
                             <button className="btn btn-error btn-circle" onClick={() => setLobby((prev) => {
                                const newLobby = [...prev];
                                newLobby[i] = { ...newLobby[i], isMuted: !newLobby[i].isMuted };
                                return newLobby;
                             })} ><BiMicrophone className="size-6" /></button>}
                        ...
                        <div className="alert alert-info mt-2">
                            <ImWarning />
                            <p>Every question you get right increases your coconut multiplier. Every wrong answer decreases it. Choose wisely.</p>
